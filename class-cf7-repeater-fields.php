...
class CF7_Repeater_Fields {
	...
	public function shortcode_render( $atts, $content ) {
		...
		if ( empty( $group_id ) ) {
			return sprintf(
				...
				'<p>' . __( 'You need to set an ID to this group. Use %s format.', 'cf7-repeater-fields' ) . '</p>',
	...
	public function wpcf7_enqueue_scripts() {
		...
		wp_enqueue_script(
			...
			plugin_dir_url( CF7_Repeater_Fields_FILE ) . 'assets/js/scripts' . $suffix . '.js',
			array( 'jquery' ),
			CF7_Repeater_Fields_VERSION,
			true
		);
	}
